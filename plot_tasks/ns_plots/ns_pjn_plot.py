...
def data_process_func(p_ang_ls_res, data, vtime = False): # should be trimmed.
    ...
    ang_ls = [t_dic[t][0] for t_dic in angle_dic.values() if t_dic['is_sharing']==False for t in t_ls] # pooled for is_sharing
    if vtime == False:
        return ang_ls
    else:
        return angle_dic
...
def ns_pjn_plot(single=True, arms=4, temp=30, conc=0.5, sp_suffix='', conf_suffix='', flag_suffix='', dims_ls= [20,2,7]):
    varname = 'pjn'
    ...
    #### plot confs ####
    x_var = rf'Patch Angles (False) of Junction ($^\circ$)'
    x_lim = (0,360)
    y_lim = (0,0.17)
    bin_num = 50
    text_pos = (10, 0.12)
