...
from plot_tasks.ns_plots import ns_k2_plot as ns_k2_plot
...
def summ_plot_k2(conf_suffix, dims_ls, conc_list, temp_list, arm_num_list, task_list, color_list, marker_list):
    ...
    su_path = f'{savepath}-k2.sudic' # (p_angs_dic, ns_tm, ns_last, sys)
        ...
        for arm_num in arm_num_list:
            for conc in conc_list:
                for temp in temp_list:
                    m1, std, m3_s = ns_k2_plot(single=True, arms=arm_num, temp=temp, conc=conc, conf_suffix=conf_suffix, dims_ls=dims_ls)
        ...
        for j,arm_num in enumerate(arm_num_list):
            ...
            for conc, color, marker in zip(conc_list, color_list, marker_list):
                ...
                if j == 0: # first subplot of a row
                    ...
                    axs[i, j].set_ylabel(rf'{task} of k2 values')
                else: # sync x,y axes across a row
                    axs[i, j].sharex(axs[i, 0])
                    axs[i, j].sharey(axs[i, 0])
    axs[0,len(arm_num_list)-1].legend()
    # plt.suptitle(f'{conf_suffix[1:]} group') # specific _jun function taking care.
    # special tasks
    # 1. a horizontal dashed line at 5/32 for all mean
    for i in range(len(arm_num_list)):
        axs[0,i].plot((15,55), (5/32,5/32),c='#1AA555',ls=':')
    # special tasks ends
    chkdir(os.path.dirname(f'{savepath}-k2.png'))
    plt.savefig(f'{savepath}-k2.png',dpi=500)
    plt.clf()
    return True
